...
class ImagesApiPath(unittest.TestCase):
    ...
    def test_http_error_surfaces(self):
        with mock.patch("httpx.post", return_value=_Resp(401, {"error": {"message": "Invalid token"}})):
...
class ChatModalityFallback(unittest.TestCase):
    ...
    def test_chat_no_image_clear_error(self):
        seq = [_Resp(405, None, ""), _Resp(200, {"choices": [{"message": {"content": "sorry, text only"}}]})]
        with mock.patch("httpx.post", side_effect=seq):
            with self.assertRaises(ImageGenError) as ctx:
...
class BaseUrlResolution(unittest.TestCase):
    def test_falls_back_to_catalog_base_url(self):
        fake_api = {"id": "openrouter", "base_url": "https://openrouter.ai/api/v1"}
        with mock.patch("model_registry.load_model_catalog", return_value={"apis": [fake_api]}), \
            ...
            openai_compat.generate("x", {}, api_id="openrouter", model="m", api_key="k", base_url=None)
        self.assertTrue(p.call_args.args[0].startswith("https://openrouter.ai/api/v1/"))
